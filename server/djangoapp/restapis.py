...
def get_dealer_reviews_from_cf(url, dealerId):
    ...
    if json_result:
        ...
        for entry in json_result:
            # Assuming each entry is a dictionary containing review data
            # Access the necessary fields to create a DealerReview object
            review_obj = DealerReview(
                dealership=entry.get("dealership"),
                name=entry.get("name"),
                purchase=entry.get("purchase"),
                review=entry.get("review"),
                purchase_date=entry.get("purchase_date"),
                car_make=entry.get("car_make"),
                car_model=entry.get("car_model"),
                car_year=entry.get("car_year"),
                sentiment=analyze_review_sentiments(entry.get("review")),
                id=entry.get("id"),
            )
...
def get_dealer_by_id(dealer_id):
    base_url = "http://127.0.0.1:3000/dealerships/get?id={}".format(dealer_id)
    ...
    if response.status_code == 200:
        ...
        print("Failed to retrieve dealer details. Status code:", response.status_code)
        return None